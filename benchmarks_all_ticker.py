...
"""
The code adopts volatility baselines presented in the respected paper, evaluated on all Tickers. Nothe that the code is a variant of basel.py, and hence the tickers are defined similarly, yet there is a conceptual difference. 
...
# Full ticker universe (all NASDAQ-100 names in sample, after renamings/delistings)
ALL_TICKERS = sorted(set(TEST_TICKERS + TRAIN_TICKERS))

# For the "full-universe" baseline version, we will evaluate on ALL_TICKERS:
EVAL_TICKERS = ALL_TICKERS
...
def har_rv_forecast(rv_series: pd.Series) -> pd.Series:
    ...
    if len(rv) < 30:
        raise RuntimeError("HAR: not enough data in RV series.")
    ...
    if df.empty:
        raise RuntimeError("HAR: no rows after lag/rolling/dropna.")
    ...
    if est_mask.sum() < 30:
        raise RuntimeError("HAR: not enough data in train+val.")
    ...
    if not test_dates:
        raise RuntimeError("HAR: no valid test dates after MIN_HISTORY_DAYS check.")
...
def train_global_lstm_on_rv(rv_daily: pd.DataFrame) -> pd.DataFrame | None:
    ...
    primary_train_universe = [t for t in ALL_TICKERS if t in rv_log.columns]
    if len(primary_train_universe) > 0:
        train_universe = primary_train_universe
    else:
        train_universe = [t for t in ALL_TICKERS if t in rv_log.columns]
...
def evaluate_ticker(
    ...
    try:
        ...
        print(f"  HAR-RV failed for {ticker}: {e}")
...
def main():
    ...
    # Evaluate baselines for each ticker
    all_results = []
